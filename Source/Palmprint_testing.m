...
clc;
tic;
tr_im=500;   % no of candidates
no_im=(500*4); %to tal no of images (4 each)
i=1;
for f=1:tr_im;
for j=1:2
%taking location of stored image, user can store it according to location
str1=strcat('E:\Database\palmprint\First Year\PPRINT (',num2str(f),')\',num2str(j+5),'.jpg');
a=imread(str1);
[m,n,z]=size(a);
...
% s24{i}=std(M24{i});Fea(i,17)=mean(s24{i});%changed to 17 keep in mind
...
fid = fopen('C:\Users\ADITYA\Desktop\ROI\features_palmprint_1000_test.txt','w');%%%%%%%%creating file to store the feature vector
fprintf(fid,'%f \n',Fea);
fclose(fid);
