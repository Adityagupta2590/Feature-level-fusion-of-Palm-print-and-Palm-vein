%%%%%%%%%%%%%%%In this program testing is performed on datasets of palmvein
...
clc;
tic;
tr_im=500;  %%%%%%no of candidate for test
no_im=(500*4); %%%%%%%%%No of image of image 
i=1;
for f=1:tr_im

for j=1:2
%%%%%%%%taking location of database   
str1=strcat('E:\Database\palm vein\first year\PVEIN (',num2str(f),')\',num2str(j+5),'.jpg');
a=imread(str1);
[m,n,z]=size(a);
bc=imresize(a,[64,64]);
if(z==3)
...
s36{i}=std(M35{i});Fea(i,36)=mean(s36{i});
...
%%%%%%%%creating file to store the feature vector
fid = fopen('C:\Users\ADITYA\Desktop\ROI\features_palmvien_1000_test.txt','w');
fprintf(fid,'%6.4f \n',Fea);
fclose(fid);
