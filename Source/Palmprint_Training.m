%%%%%%%%%%%%%%%In this program traing is performed on datasets of palmprint
...
clear all;
close all;
no_im=500;    %%no of candidate                  %%%%%%%%%%%%%%%%%%%%%555ME

i=1;
for f=1:no_im 
for j=1:4 %%%%4 image per candidate
%taking location of stored image, user can store it according to location
str1=strcat('E:\Database\palmprint\First Year\PPRINT (',num2str(f),')\',num2str(j+1),'.jpg');
a=imread(str1);%%%%%%%storing image in variable a
b=rgb2gray(a);
c=imresize(a,[64,64]);%%%%%%%%%%%resizing it to 64*64
b=rgb2gray(c);                   %%%%%%%%%%%%%%%%%%%for my data base rgb function%%%%%%%%%%%%%%%%%%%%%5
...
s25{i}=std(M25{i});Fea(i,25)=mean(s25{i});%changed to 1 from 25
...
fid = fopen('C:\Users\ADITYA\Desktop\ROI\features_palmprint_1000.txt','w');%%%%%%%%creating file to store the feature vector
fprintf(fid,'%f \n',Fea);
fclose(fid);